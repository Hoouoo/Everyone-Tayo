...
public class NearBusStopController {
    ...
    public ResponseEntity callApiWithJson(@RequestBody RequestNearBusDto requestNearBusDto) {
        ...
        List<String> targetRoute = new ArrayList<>();
        busRouteRepository.findAllByRouteNo(requestNearBusDto.getBusNumber()).forEach(item -> targetRoute.add(item.getRouteId()));
        System.out.println("targetRoute = " + targetRoute.get(0));
